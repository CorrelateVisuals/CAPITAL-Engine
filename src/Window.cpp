...
Window::Window() : window{nullptr}, framebufferResized{false} {
  LOG.console("{ [X] }", "constructing Window");
  initWindow();
}

Window::~Window() {
  LOG.console("{ [X] }", "destructing Window");
...
void Window::initWindow() {
  ...
  LOG.console("{ [*] }", "Window initialized with", displayConfig.width, "*",
...
void Window::mouseClick(GLFWwindow* window, int button) {
  ...
  if (oldState == GLFW_PRESS && newState == GLFW_RELEASE) {
    ...
    LOG.console("{ in }", "Mouse Click at", xpos, ":", ypos);
  }
  oldState = newState;
}
